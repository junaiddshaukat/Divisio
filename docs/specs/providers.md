...
Settings → Usage reads session files for the agents Divisio drives when those homes exist: Claude Code (`~/.claude/projects`), Codex (`$CODEX_HOME/sessions`), Cursor (`state.vscdb` composer bubbles), Grok (`~/.grok/sessions/**/updates.jsonl`), Qwen (`~/.qwen/usage`), OpenCode (`opencode.db`). Headline is processed tokens: uncached input + cache read + cache write + output. Grok only exposes a cumulative total, so those rows are turn deltas without an invented input/output split. Cache hits are often the bulk of the Claude/Codex number. Divisio-orchestrated `turn.usage` is shown as a footnote and is not added on top. It is not billed spend and not a vendor quota. Community agents without on-disk counters still appear as unmetered when they ran through Divisio.
