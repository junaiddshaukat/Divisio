...
import { normalizeGeminiStreamLine, type GeminiNormalizeState } from "./gemini/normalize.ts";
...
export class GeminiAdapter implements ProviderAdapter {
  ...
  async detect(): Promise<DetectResult> {
    return detectCli(
      BINARY,
      ["--version"],
      "gemini not on PATH — npm i -g @google/gemini-cli",
      "gemini exited non-zero — try `gemini` auth / login",
    );
  }
  ...
  async sendTurn(handle: SessionHandle, turn: SendTurnInput): Promise<void> {
    ...
    const initial: GeminiNormalizeState = { nativeId: session.nativeId, seenTools: new Set() };
    void pumpCommunityNdjson({
      ...
      normalize: normalizeGeminiStreamLine,
      onNativeId: (id) => {
        session.nativeId = id;
      },
